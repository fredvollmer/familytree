...
for (const [fileName, sheetName] of [
  ...
  ['Fredric_Vollmer_Complete_Family_Tree_Media_Inventory.csv', 'Media Audit'],
]) {
...
overview.getRange('A1').values = [
  ['Fredric Vollmer Complete Family Tree — Consolidated Index'],
];
overview.getRange('A2:F2').merge();
overview.getRange('A2').values = [
  [
    'One source-of-truth package assembled from the standardized maternal tree and all completed project-chat research',
  ],
];
overview.getRange('A4:B4').values = [['Tree metric', 'Current value']];
overview.getRange('A5:A17').values = [
  ['Individuals'],
  ['Families'],
  ...
  ['People photo-audited'],
  ['External photo pages located'],
...
overview.getRange('A19:F19').values = [
  [
    'Package component',
    'Role',
    'Rows / items',
...
overview.getRange('A20:F28').values = [
  [
    'GEDCOM',
  ...
  [
    ...
    8,
    'Original tabs in this workbook',
    'Preserved',
...
for (const address of ['A4:B4', 'D4:F4', 'A19:F19']) {
...
overview.getRange('A5:B17').format.fill = pale;
overview.getRange('B5:B17').format = {
...
overview.getRange('A20:F28').format = {
  wrapText: true,
  verticalAlignment: 'top',
};
overview
  .getRange('A20:F28')
  .conditionalFormats.addCustom('=MOD(ROW(),2)=0', { fill: '#EAF3F8' });
overview.getRange('A1:F1').format.font = {
  name: 'Aptos Display',
  size: 16,
  bold: true,
  color: '#FFFFFF',
};
overview.getRange('A2:F2').format.font = {
  name: 'Aptos',
  size: 11,
  bold: true,
  color: navy,
...
overview.getRange('A4:F28').format.rowHeight = 28;
overview.getRange('A20:F28').format.rowHeight = 46;
overview.getRange('A1:A28').format.columnWidth = 29;
overview.getRange('B1:B28').format.columnWidth = 24;
overview.getRange('C1:C28').format.columnWidth = 13;
overview.getRange('D1:D28').format.columnWidth = 46;
overview.getRange('E1:E28').format.columnWidth = 19;
overview.getRange('F1:F28').format.columnWidth = 50;
overview.freezePanes.freezeRows(2);

const configs = [
  {
    name: 'Consolidated People',
    table: 'ConsolidatedPeopleTable',
    widths: [14, 32, 7, 30, 30, 66, 30, 26, 72, 18, 24],
  },
  {
    name: 'Consolidated Families',
    table: 'ConsolidatedFamiliesTable',
    widths: [14, 14, 14, 48, 30, 72, 28],
  },
  {
    name: 'Consolidated Sources',
    table: 'ConsolidatedSourcesTable',
    widths: [18, 46, 26, 16, 80, 28],
  },
  {
    name: 'Vital Date Coverage',
    table: 'VitalDateCoverageTable',
    widths: [14, 32, 30, 35, 30, 35, 32, 28],
  },
  {
    name: 'Occupation Coverage',
    table: 'OccupationCoverageTable',
    widths: [14, 32, 78, 13, 38, 26, 26, 26, 72],
  },
  {
    name: 'Media Audit',
    table: 'MediaAuditTable',
    widths: [15, 14, 42, 32, 48, 52, 36, 36, 24, 22, 60, 80],
  },
];
for (const config of configs) {
  ...
  config.widths.forEach((width, index) => {
    sheet.getRangeByIndexes(0, index, used.rowCount, 1).format.columnWidth =
      width;
  });
